...
\subsection{Inspection Visuelle : Qualité des Échantillons Générés}

Pour corroborer les métriques quantitatives (NLL et unités actives), il est impératif de procéder à une inspection qualitative des capacités génératives du modèle. Nous avons généré des échantillons aléatoires à partir du prior $z \sim \mathcal{N}(0, I)$ pour les deux modèles entraînés.

La Figure~\ref{fig:samples_comparison} présente une comparaison directe entre les chiffres générés par le VAE standard ($K=1$) et ceux produits par l'IWAE ($K=50$).

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \textbf{(a) VAE Standard ($K=1$)} \\
        \vspace{0.2cm}
        % Assure-toi que l'image est bien dans le dossier pic/
        \includegraphics[width=0.8\textwidth]{pic/vae_sample.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \textbf{(b) IWAE ($K=50$)} \\
        \vspace{0.2cm}
        \includegraphics[width=0.8\textwidth]{pic/iwae_sample.png}
    \end{minipage}
    \caption{Comparaison qualitative des échantillons générés. (a) Le VAE produit des chiffres cohérents mais souffre parfois de discontinuités ou de flou sur les bords. (b) L'IWAE génère des traits plus nets, avec un contraste plus marqué entre le fond et la forme, témoignant d'une meilleure modélisation de la densité marginale.}
    \label{fig:samples_comparison}
\end{figure}

L'analyse visuelle révèle des différences subtiles mais significatives qui confirment nos hypothèses théoriques :

\begin{itemize}
    \item \textbf{Netteté des contours :} Comme anticipé dans la section précédente concernant le flou variationnel, les échantillons du VAE présentent davantage de pixels gris intermédiaires (incertitude du modèle), résultant en des chiffres aux contours parfois "baveux". À l'inverse, l'IWAE tend à produire des transitions plus franches (pixels noirs ou blancs), signe d'une confiance accrue du décodeur.
    
    \item \textbf{Cohérence topologique :} On observe sur les échantillons VAE quelques artefacts de discontinuité (par exemple, des boucles de '8' non fermées ou des barres de '5' fragmentées). L'IWAE, bénéficiant d'une borne plus serrée, capture mieux les dépendances à longue portée dans l'espace des pixels, réduisant la fréquence de ces topologies aberrantes.
    
    \item \textbf{Diversité des styles :} Bien que plus difficile à quantifier sans métrique de type \textit{FID Score}, l'IWAE semble explorer une variété légèrement supérieure de styles calligraphiques (inclinaison, épaisseur du trait), ce qui est cohérent avec l'observation d'un nombre plus élevé d'unités latentes actives. Le VAE, contraint par sa KL-divergence restrictive, a tendance à se concentrer sur les "prototypes" moyens des chiffres, réduisant la variance intra-classe des générations.
\end{itemize}

Cette inspection qualitative valide que l'amélioration de la borne n'est pas un artifice numérique, mais se traduit concrètement par une meilleure synthèse d'images.
