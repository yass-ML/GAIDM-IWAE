...
Soit $I \subset \{1, \dots, k\}$ avec $|I| = m$ un sous-ensemble uniformément
distribué d'indices distincts de $\{1, \dots, k\}$. Nous utiliserons
l'observation simple suivante : $\mathbb{E}_{I=\{i_1, \dots, i_m\}} \left[
\frac{a_{i_1} + \dots + a_{i_m}}{m} \right] = \frac{a_1 + \dots + a_k}{k}$ pour
toute suite de nombres $a_1, \dots, a_k$.
...
L'objectif IWAE implique à la fois les paramètres génératifs $\theta$ (décodeur) et les paramètres d'inférence $\phi$ (encodeur). Le gradient par rapport à $\phi$ peut s'écrire en utilisant l'astuce de reparamétrisation:
\begin{equation}
