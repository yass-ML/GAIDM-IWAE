...
\begin{frame}{Result 1: Estimated Log-Likelihood vs Number of Samples (K)}
    ...
    \begin{figure}[htpb]
        ...
        \begin{tikzpicture}
            ...
            % IWAE (K>1) in blue
            \addplot[fill=myBlue!70, draw=myBlue!90] coordinates {
                (K=5, -78.36)
                (K=20, -77.34)
                (K=30, -77.12)
                (K=50, -76.58)
...
\begin{frame}{Analysis: Impact of K on Complexity}
    \begin{columns}
        \column{0.5\textwidth}
        \centering
        \textbf{Training Time vs. K}
        \begin{tikzpicture}[scale=0.6]
            \draw[->] (0,0) -- (4,0) node[right] {$K$};
            \draw[->] (0,0) -- (0,4) node[above] {Time / Memory};
            \draw[blue, thick] (0,0) -- (3.5, 3.5); % Linear slope
            \node[below] at (1,0) {1};
            \node[below] at (2,0) {5};
            \node[below] at (3.5,0) {20};
            \fill[blue] (1,1) circle (2pt);
            \fill[blue] (2,2) circle (2pt);
            \fill[blue] (3.5,3.5) circle (2pt);
            \node[anchor=north west, blue] at (0.5, 3) {Linear Growth $O(K)$};
        \end{tikzpicture}

        \column{0.5\textwidth}
        \textbf{Trade-off Analysis:}
        \begin{itemize}
            \item \textbf{Compute:} Training time and memory usage scale \textbf{linearly} with $K$.
            \item \textbf{Performance:} Returns diminish as $K$ increases (gap between $K=5$ and $K=20$ is smaller than $1 \to 5$).
            \item \textbf{Gradient Variance:} Higher $K$ leads to lower variance gradients (DReG estimator), stabilizing training.
        \end{itemize}
    \end{columns}
\end{frame}
